...
class User:
    ...
    def get_all_user_info(cls, data):
        query= 'SELECT *, posts.id as postId FROM users LEFT JOIN posts on posts.user_id = users.id WHERE users.id = %(user_id)s;'
        results =  connectToMySQL(cls.db_name).query_db(query, data)
        posts = []
        for row in results:
            posts.append(row)
    ...
    def get_logged_user_liked_posts(cls, data):
        query = 'SELECT likes.post_id as id FROM likes LEFT JOIN users on likes.user_id = users.id WHERE likes.user_id = %(user_id)s;'
        results = connectToMySQL(cls.db_name).query_db(query, data)
        postsLiked = []
        for row in results:
